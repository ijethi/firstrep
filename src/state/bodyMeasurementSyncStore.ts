...
/**
 * Body-measurement sync status store (B-24). Only `lastSyncedAtISO` persisted;
 * `status`/`lastError` transient. Holds UI status only, never measurement data.
 */
interface BodyMeasurementSyncState {
  status: SyncStatus;
...
export const useBodyMeasurementSyncStore = create<BodyMeasurementSyncState>()(
  persist(
    (set) => ({
      status: 'idle',
    ...
    {
      name: STORAGE_KEYS.bodyMeasurementSync,
