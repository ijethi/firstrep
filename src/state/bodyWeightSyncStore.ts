...
/**
 * Body-weight sync status store (B-23). Only `lastSyncedAtISO` persisted;
 * `status`/`lastError` transient. Holds UI status only, never weight data.
 */
interface BodyWeightSyncState {
  status: SyncStatus;
...
export const useBodyWeightSyncStore = create<BodyWeightSyncState>()(
  persist(
    (set) => ({
      status: 'idle',
    ...
    {
      name: STORAGE_KEYS.bodyWeightSync,
