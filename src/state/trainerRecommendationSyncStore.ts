...
/**
 * Trainer-recommendation sync status store (B-26). Only `lastSyncedAtISO`
 * persisted; `status`/`lastError` transient. Holds UI status only.
 */
interface TrainerRecSyncState {
  status: SyncStatus;
...
export const useTrainerRecSyncStore = create<TrainerRecSyncState>()(
  persist(
    (set) => ({
      status: 'idle',
    ...
    {
      name: STORAGE_KEYS.trainerRecSync,
