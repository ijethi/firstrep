...
/**
 * Weekly-check-in sync status store (B-25). Only `lastSyncedAtISO` persisted;
 * `status`/`lastError` transient. Holds UI status only, never check-in data.
 */
interface WeeklyCheckInSyncState {
  status: SyncStatus;
...
export const useWeeklyCheckInSyncStore = create<WeeklyCheckInSyncState>()(
  persist(
    (set) => ({
      status: 'idle',
    ...
    {
      name: STORAGE_KEYS.weeklyCheckInSync,
