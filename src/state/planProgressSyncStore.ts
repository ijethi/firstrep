...
/**
 * Plan-progress sync status store (B-20). Only `lastSyncedAtISO` persisted;
 * `status`/`lastError` transient. Holds UI status only, never the progress itself.
 */
interface PlanProgressSyncState {
  status: SyncStatus;
...
export const usePlanProgressSyncStore = create<PlanProgressSyncState>()(
  persist(
    (set) => ({
      status: 'idle',
    ...
    {
      name: STORAGE_KEYS.planProgressSync,
