...
import { syncTrainerRecommendations } from '../lib/trainerRecommendationSync';

const SCOPE_COPY =
  'Only trainer recommendations sync here. Progress photos stay on this device for now.';
...
/** Manual "Sync trainer recommendations" control + status. Disabled when signed out. */
export default function TrainerRecommendationSyncCard() {
  const user = useAuthStore((s) => s.user);
  const authStatus = useAuthStore((s) => s.status);
  const status = useTrainerRecSyncStore((s) => s.status);
  const lastError = useTrainerRecSyncStore((s) => s.lastError);
  const lastSyncedAtISO = useTrainerRecSyncStore((s) => s.lastSyncedAtISO);

  const canSync = authStatus === 'signed_in' && !!user;

  return (
    <View style={styles.card}>
      <Text style={styles.title}>🧠 Trainer tips sync</Text>
      ...
      <AppButton
        label="Sync trainer recommendations"
        variant="secondary"
        onPress={() => syncTrainerRecommendations(user)}
