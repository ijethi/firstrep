...
const SCOPE_COPY =
  'Only your plan progress syncs here. Workout history still stays on this device.';
...
/** Manual "Sync plan progress" control + status. Disabled when signed out. */
export default function PlanProgressSyncCard() {
  const user = useAuthStore((s) => s.user);
  const authStatus = useAuthStore((s) => s.status);
  const status = usePlanProgressSyncStore((s) => s.status);
  const lastError = usePlanProgressSyncStore((s) => s.lastError);
  const lastSyncedAtISO = usePlanProgressSyncStore((s) => s.lastSyncedAtISO);

  const canSync = authStatus === 'signed_in' && !!user;

  return (
    <View style={styles.card}>
      <Text style={styles.title}>📍 Plan progress sync</Text>
      ...
      <AppButton
        label="Sync plan progress"
        variant="secondary"
        onPress={() => syncPlanProgress(user)}
